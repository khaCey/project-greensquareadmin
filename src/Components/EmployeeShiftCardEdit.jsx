        ...
        {shifts.map((shift, index) => {
            ...
            const totalHoursWorked =
                clockInTime && clockOutTime
                    ? (clockOutTime - clockInTime) / (1000 * 60 * 60) // Convert to hours
                    : 0;
            ...
            return (
                <ShiftCardStyled key={index}>
                    <ShiftDate>
                        {clockInTime ? clockInTime.toDateString() : 'N/A'}
                    </ShiftDate>
                    <ShiftClock>
                        {clockInTime ? clockInTime.toLocaleTimeString() : 'N/A'}
                    </ShiftClock>
                    <ShiftClock>
                        {clockOutTime ? clockOutTime.toLocaleTimeString() : 'N/A'}
                    </ShiftClock>
                    <ShiftTotal>{decimalToHrsMins(workedHours)}</ShiftTotal>
                    <ShiftBreak>{decimalToHrsMins(totalBreakTime)}</ShiftBreak>
                </ShiftCardStyled>
            );
        })}
    </ShiftCardContainer>
